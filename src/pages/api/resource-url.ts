...
export const GET: APIRoute = async (context) => {
  ...
  const { data: resource } = await supabase
    .from('resource')
    .select('id, name, storage_path, course_id')
    .eq('id', resourceId)
    .single()
  ...
  // 3. Mint a short-lived signed URL to the private storage object
  ...
  return new Response(
    JSON.stringify({ url: signedUrlData.signedUrl, name: resource.name }),
    {
      status: 200,
